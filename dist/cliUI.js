...
function printConfigSummary(config, envPath) {
    console.log();
...
class UploadProgressBar {
    ...
    failed = 0;
    lastName = "";
    lastStaticRenderCompleted = -1;
    lastStaticRenderFailed = -1;
    constructor(options) {
    ...
    finish() {
        ...
        const summaryTone = summary.failed > 0 ? "warning" : "success";
    ...
    render() {
        ...
        const shouldRender = this.completed === this.options.total ||
    ...
    progressLine() {
        ...
        const currentFile = this.lastName
            ? ` ${dim(truncate(path_1.default.basename(this.lastName), 24))}`
            : "";
